...
controllers, and the calibration tool. Concrete example: re-enabling
the 3-joint neck on Lite, which was dropped at first-bringup time.
...
| `bar_bringup_lite/config/calibration.json` | `homing_offset` for the new joint (created via [Calibrate the zero pose](./calibrate_zero_pose.md)) |
...
Verify the URDF expands cleanly:
...
`./calibration.json` over `bar_bringup_lite/config/calibration.json`.
...
# Should now see 14 + new joints in /joint_states
ros2 topic echo --once /joint_states | grep -c " - " # name count
```
...
JSON has an entry with the expected joint name.
...
- The `joint_names` array in `MITAction` messages must match the
