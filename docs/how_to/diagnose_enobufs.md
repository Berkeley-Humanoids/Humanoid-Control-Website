...
```
[bar_hw_socketcan]: CAN write() returned -1 (errno=No buffer space available) for id=0x017FFF0B
[bar_hw_socketcan]: CAN write() returned -1 (errno=No buffer space available) for id=0x017FFF15
...
```
...
ros2 run bar_hw_robstride robstride_discover --iface can1
# Should report 7 motors on each bus, no ENOBUFS warnings in the output.
```

If `robstride_discover` itself reports `tx_dropped > 0`, motors are
...
├── robstride_discover reports 0 motors → motor power off → fix power, relaunch
├── robstride_discover reports motors but ENOBUFS persists at activation only
