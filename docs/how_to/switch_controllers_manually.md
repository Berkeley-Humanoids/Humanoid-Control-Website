...
`remote_policy_controller` immediately starts looking for
`MITAction` on `/remote_policy_controller/command`. Without a
...
`bar_policy/remote_policy_runner` first.

For `rl_policy_controller`, see the caveat in
[Troubleshooting](../reference/troubleshooting.md#failed-to-configure-controller-rl_policy_controller).
