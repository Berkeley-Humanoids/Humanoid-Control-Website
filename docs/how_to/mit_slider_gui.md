...
- A `forward_command_controller/MultiInterfaceForwardCommandController`
  ...
  `bar_hw_robstride/test/single_robstride_gui_controllers.yaml` is
  the canonical example.

## Step 1 — Launch the test stack

For a single-motor sanity check there's a dedicated launch:
...
2. Load a forward command controller separately:
...
- The GUI source: [`bar_hw_robstride/scripts/mit_slider_gui.py`](https://github.com/T-K-233/bar_ros2/blob/main/bar_hw_robstride/scripts/mit_slider_gui.py).
