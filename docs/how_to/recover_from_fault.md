...
1. Confirm motors are powered:
   `ros2 run bar_hw_robstride robstride_discover --iface canN`
...
2. If the fault persists, single-step diagnosis:
   ```bash
   ros2 run bar_hw_robstride robstride_ping --iface canN --id <X> --read-status
   ```
...
# Walk back:
#  L1+A on the gamepad   →   STANDBY
#  R1+A on the gamepad   →   REMOTE (or whatever your task is)
```
