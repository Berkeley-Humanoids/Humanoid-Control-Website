...
- The `bar_hw_robstride/RobstrideSystem` lifecycle.
...
```bash
ros2 run bar_hw_robstride robstride_ping --iface can0 --id <X>
# TX  GetDeviceId  id=...
...
```bash
ros2 run bar_hw_robstride robstride_discover --iface can0 --scan-to 127
```
...
```bash
ros2 run bar_hw_robstride robstride_ping --iface can0 --id <X> --read-status
...
`bar_hw_robstride` ships a self-contained launch for this:

```bash
ros2 launch bar_hw_robstride single_robstride_gui.launch.py \
    can_id:=<X>
```

This brings up:
- A 1-joint URDF describing your motor (`single_robstride_test`).
- `ros2_control_node` loading `bar_hw_robstride/RobstrideSystem`.
...
Then run the same `robstride_ping --iface vcan0` and
`robstride_discover --iface vcan0`. You won't get replies (nothing's
listening), but the TX path lights up and you can see your own
frames with `candump vcan0`. The single-motor launch will boot but
report `RX_TIMEOUT` in `/safety_status`.
