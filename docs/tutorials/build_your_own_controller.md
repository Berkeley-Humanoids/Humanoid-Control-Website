...
```bash
cd ~/bar_ws
colcon build --packages-select bar_controllers --symlink-install
...
In another terminal, load the controller via the CLI:
...
| `bar/RemotePolicyController` | Subscribes to an `MITAction` topic; `RealtimeBuffer` for the RT handoff; stale-command policy. |
