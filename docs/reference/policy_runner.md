...
| `PianoKeyReferenceProvider` | Subscribes to `/piano/key_command` (published by `bar_piano/midi_replay_node`) with `RELIABLE` + `TRANSIENT_LOCAL` + `KEEP_LAST(1)` QoS and serves `key_goal_states` (current 88-key float vector) and `key_goal_states_lookahead` (flat `K × 88`). Activated when `task_type='piano'`. The publisher owns song timing, so the provider's `reset` / `step` hooks are intentional no-ops — the latched value follows the most recent publish, not a shadow counter on this side. |
