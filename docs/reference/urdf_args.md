...
| `can_interface_left`  | `can0` | SocketCAN ifname for the `LiteLeftArm` block (real path only) |
| `can_interface_right` | `can1` | SocketCAN ifname for the `LiteRightArm` block (real path only) |
| `calibration_file`    | `""` | Absolute path to the per-physical-robot calibration JSON. Empty = identity calibration. |
...
(both false)                         -> bar_hw_robstride/RobstrideSystem
```
...
<ros2_control name="LiteLeftArm" type="system">
  <hardware>
    <plugin>bar_hw_robstride/RobstrideSystem</plugin>
...
all 14 joints (no `can_interface`, no `calibration_file`).
...
| `calibration_file` | string | `""` | JSON calibration path; empty = identity |
...
xacro $(ros2 pkg prefix bar_description_lite)/share/bar_description_lite/urdf/lite.urdf.xacro \
    ...
    calibration_file:=/abs/path/to/calibration.json \
    > /tmp/lite_real.urdf
```
